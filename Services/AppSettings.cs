...
{
    /// <summary>
    /// Application settings service with duplicate folder filter options
    /// </summary>
    public class AppSettings : INotifyPropertyChanged
    {
        ...
        // Duplicate folder filter settings
        ...
        /// <summary>
        /// Enables or disables duplicate folder filters
        /// </summary>
        public bool EnableDuplicateFilters
        {
            get => _enableDuplicateFilters;
            set
            {
                if (_enableDuplicateFilters != value)
                {
                    _enableDuplicateFilters = value;
                    OnPropertyChanged();
                    Save();
                }
            }
        ...
        /// <summary>
        /// Minimum folder name length for duplicate detection. Folders with names shorter than this will be excluded.
        /// </summary>
        public int MinFolderNameLength
        {
            get => _minFolderNameLength;
            set
            {
                int newValue = ValidateRange(value, 1, 50);
                if (_minFolderNameLength != newValue)
                {
                    _minFolderNameLength = newValue;
                    OnPropertyChanged();
                    Save();
                }
            }
        ...
        /// <summary>
        /// List of folder names to exclude from duplicate detection (case-insensitive)
        /// </summary>
        public List<string> ExcludedFolderNames
        {
            ...
            set
            {
                _excludedFolderNames = value ?? new List<string>();
                OnPropertyChanged();
                Save();
            }
        }
        ...
        /// <summary>
        /// Adds a folder name to the exclusion list for duplicate detection
        /// </summary>
        /// <param name="folderName">Name of the folder to exclude (case-insensitive)</param>
        public void AddExcludedFolderName(string folderName)
        {
            if (string.IsNullOrWhiteSpace(folderName))
                return;

            // Create a new list to trigger property change
            var updatedList = new List<string>(_excludedFolderNames);

            // Check if already exists (case-insensitive)
            if (!updatedList.Exists(name => string.Equals(name, folderName, StringComparison.OrdinalIgnoreCase)))
            {
                updatedList.Add(folderName.Trim());
                ExcludedFolderNames = updatedList;
            }
        }

        /// <summary>
        /// Removes a folder name from the exclusion list
        /// </summary>
        /// <param name="folderName">Name of the folder to remove from exclusion list</param>
        public void RemoveExcludedFolderName(string folderName)
        {
            if (string.IsNullOrWhiteSpace(folderName))
                return;
            ...
            updatedList.RemoveAll(name => string.Equals(name, folderName, StringComparison.OrdinalIgnoreCase));
            ExcludedFolderNames = updatedList;
        }

        /// <summary>
        /// Checks if a folder name should be excluded from duplicate detection
        /// </summary>
        /// <param name="folderName">Name of the folder to check</param>
        /// <returns>True if the folder should be excluded</returns>
        public bool IsFolderNameExcluded(string folderName)
        {
            if (string.IsNullOrWhiteSpace(folderName) || !EnableDuplicateFilters)
                return false;

            // Check minimum length filter
            if (folderName.Length < MinFolderNameLength)
                return true;

            // Check exclusion list
            return _excludedFolderNames.Exists(name =>
